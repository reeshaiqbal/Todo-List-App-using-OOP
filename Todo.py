class TodoList:  # I created a class and enclosed "data" and "methods" in it implementing encapsulation
    ...
    def add_task(self):
        ...
        # Used dictionary for better readability of tasks
        self.tasks.append({"title": add, "done": False})
        ...
        if len(self.tasks) == 0:
            ...
            for i in range(len(self.tasks)):
                if self.tasks[i]["done"] == True:
                    ...
                    status = "Pending"
                print(i+1, self.tasks[i]["title"], status)
    ...
    def search_task(self):
        ...
        found = False
        for i in range(len(self.tasks)):
            if word.lower() in self.tasks[i]["title"].lower():
                if self.tasks[i]["done"] == True:
                    status = "Done"
                else:
                    status = "Pending"
    ...
    def update_task(self):
        ...
        if 1 <= number <= len(self.tasks):
            ...
            self.tasks[number-1]["title"] = new_text
